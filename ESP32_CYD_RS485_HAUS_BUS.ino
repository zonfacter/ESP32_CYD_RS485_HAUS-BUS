/*  ESP32 ST7789 Touch-Menü mit CSMA/CD
    Version 1.50 - Mai 2025
    
    Neue Features in 1.50:
    - Service-Menü mit 20-Sekunden Touch-Aktivierung
    - Device ID Konfiguration per Touch
    - Orientierungs-Umschaltung (Portrait/Landscape)
    - EEPROM-Konfigurationsspeicherung
    - Telegramm-Steuerung für Service-Modus
...
#include "backlight.h"
#include "led.h"
#include "service_manager.h"  // NEU: Service-Manager
...
void setup() {
  ...
  delay(100);
  ...
  #if DB_INFO == 1
    Serial.println("\nESP32 ST7789 Touch-Menü mit CSMA/CD + Service-Menü - Start");
    Serial.println("Firmware-Version: 1.50");
    ...
    Serial.println("NEU: Service-Menü mit 20s Touch-Aktivierung");
  #endif
  ...
  if (serviceManager.getOrientation() != SCREEN_ORIENTATION) {
    ...
    // Orientierung über public Methode setzen
  ...
  tft.drawCentreString("Version 1.50 + Service-Menü", SCREEN_WIDTH/2, SCREEN_HEIGHT/2, 2);
  tft.drawCentreString("Initialisierung...", SCREEN_WIDTH/2, SCREEN_HEIGHT/2 + 20, 1);
  tft.drawCentreString("20s Touch = Service-Modus", SCREEN_WIDTH/2, SCREEN_HEIGHT - 30, 1);
  
  // Erste Statusmeldung der Hintergrundbeleuchtung
  sendBacklightStatus();
  lastBacklightStatusTime = millis();
  
  delay(2000);  // Längere Anzeige für neue Infos
